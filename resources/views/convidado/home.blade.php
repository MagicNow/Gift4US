			...
			<div class="preview-header-container">
				<div class="preview-header">
					...
					<div class="preview-header-info">{{ $party->festa_dia }}.{{ $party->festa_mes }} | {{ date ('H:i',strtotime($party->festa_hora . ':' . $party->festa_minuto)) }}<br>
						{{ isset($party->idade_anos) && $party->idade_anos > 0 ? $party->idade_anos . ($party->idade_anos > 1 ? ' anos' : ' ano') : NULL }}
						{{ isset($party->idade_meses) && $party->idade_meses > 0 ? $party->idade_meses . ($party->idade_meses  > 1 ? ' meses' : ' mês') : NULL }}
					</div>
				</div>
			</div>
		</div>
